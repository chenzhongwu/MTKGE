...
class Data(object):
    ...
    def get_pattern_tri(self, sup_tri):
        ...
        # print(tail_head)
        rel_rel_timezheng = torch.zeros((self.num_rel, self.num_rel), dtype=torch.int)
        rel_rel_timefu = torch.zeros((self.num_rel, self.num_rel), dtype=torch.int)
        rel_rel_timetongshi = torch.zeros((self.num_rel, self.num_rel), dtype=torch.int)
        # print(rel_rel_time.size())
        # for i in range(0, num_time):
        #     for r1 in range(0, self.num_rel):
        #         if time_rel[r1][i] != 0:
        #             for r2 in range(0, self.num_rel):
        #                 if r2 != r1:
        #                     for j in range(0, self.num_time):
        #                         if time_rel[r2][j] != 0 and j < i:
        #                             rel_rel_timezheng[r1][r2] += 1
        #                         if time_rel[r2][j] != 0 and j > i:
        #                             rel_rel_timefu[r2][r1] += 1
        #                         if time_rel[r2][j] != 0 and j == i:
        #                             rel_rel_timetongshi[r1][r2] += 1
        #                             # rel_rel_timetongshi[r1][r2] += 1
        # print(rel_rel_timexiangdui)
        # print(rel_rel_timetongshi)
        # construct pattern graph from adjacency matrix
        src = torch.LongTensor([])
        dst = torch.LongTensor([])
        p_rel = torch.LongTensor([])
        src_time = torch.LongTensor([])
        dst_time = torch.LongTensor([])
        p_rel_time = torch.LongTensor([])
        ...
        # for p_rel_idx, mat in enumerate([rel_rel_timezheng, rel_rel_timefu, rel_rel_timetongshi]):
        ...
        # for p_rel_idx, mat in enumerate([tail_head, head_tail, tail_tail, head_head]):
        #     sp_mat = sparse.coo_matrix(mat)
        #     # 返回sp_mat创建稀疏矩阵时的行索引row/列索引col，对应
        #     src = torch.cat([src, torch.from_numpy(sp_mat.row)])
        #     dst = torch.cat([dst, torch.from_numpy(sp_mat.col)])
        #     # * 是指复制多少次，对应4个种类tt、hh、th、ht
        #     p_rel = torch.cat([p_rel, torch.LongTensor([p_rel_idx] * len(sp_mat.data))])
        #     # p_w代表具体每种有多少个组合，具体u的数字，这里没用到
        #     p_w = torch.cat([p_w, torch.from_numpy(sp_mat.data)])
        #
        # return torch.stack([src, p_rel, dst]).T.tolist()
